...
class MedicationSearchEngine:
    ...
    def _build_index(self) -> None:
        ...
        for med in self.medications:
            doc_lens.append(len(med.tokens))
            for token in set(med.tokens):
    ...
    def _bm25_score(self, query_tokens: List[str], med: MedicationRecord, doc_len: int) -> float:
        if not query_tokens:
            return 0.0
    ...
    def match(self, query: str, threshold: float = DEFAULT_THRESHOLD) -> Dict[str, object]:
        normalized_query = normalize_text(query)
        query_tokens = tokenize(normalized_query)

        if not normalized_query:
            return {"input": query, "matched": False, "selected": None, "candidates": [], "reason": "Empty drug name"}

        scored = []
        similarity_scored = []
        for idx, med in enumerate(self.medications):
            bm25 = self._bm25_score(query_tokens, med, self._doc_lens[idx])
        ...
        for idx, med, _ in similarity_scored[:TOP_K_RETRIEVE]:
            if idx not in candidate_pool:
                candidate_pool[idx] = (med, self._bm25_score(query_tokens, med, self._doc_lens[idx]))
        ...
        selected: Optional[Dict] = candidates[0] if candidates else None
        ...
        return {"input": query, "matched": True, "selected": selected, "candidates": candidates[:5], "reason": None}
...
def load_medications() -> List[MedicationRecord]:
    ...
    with MEDICATIONS_CSV.open("r", encoding="utf-8-sig", newline="") as f:
        ...
        for row in reader:
            generic_name = _csv_value(row, "generic_name")
            medication_group = _csv_value(row, "medication_group")
...
def load_interactions() -> List[InteractionRecord]:
    ...
    with INTERACTIONS_CSV.open("r", encoding="utf-8-sig", newline="") as f:
        reader = csv.DictReader(f)
        for row in reader:
            g1 = _csv_value(row, "group_1")
            g2 = _csv_value(row, "group_2")
            if not g1 or not g2:
                continue
            interactions.append(InteractionRecord(
                group_1=g1,
                group_2=g2,
                interaction_severity=_csv_value(row, "interaction_severity"),
                possible_symptoms=_csv_value(row, "possible_symptoms"),
                interaction_risk=_csv_value(row, "interaction_risk"),
...
def find_interactions(
    ...
    mapped_drugs: set = set()
    mapped_groups: set = set()
    ...
    # Prefer drug-drug over group-group when both exist for the same pair
    covered_group_pairs: set = set()
    ...
    for finding in findings:
        if finding["matched_by"] == "group-group":
            pair_key = "|".join(sorted([finding["norm_1"], finding["norm_2"]]))
            if pair_key in covered_group_pairs:
                continue
        result.append({
