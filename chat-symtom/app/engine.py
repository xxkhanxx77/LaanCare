...
class ChatEngine:
    ...
    def handle_message(self, user_id, text, session_mode="self_checkin", user_can_chat=True, raw_payload=None):
        ...
        if state.get("pending_rule_id") == "RED_BUNDLE":
            return self._handle_red_bundle_answer(user, session, user_id, text, state, console, usage_delta, None)
        ...
        if state.get("flow_phase") == "yellow_open" and state.get("pending_rule_id"):
            return self._handle_yellow_open_answer(user, session, user_id, text, state, console, usage_delta, None)
